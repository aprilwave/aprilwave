...
function buildPath(anchors: { x: number; y: number }[], t: number): string {
  ...
  const pts = anchors.map((a, i) => ({
    x: a.x * VW,
    y: a.y * VH + Math.sin(t * 0.00028 + i * 1.1) * 18 + Math.sin(t * 0.00051 + i * 2.3) * 8,
  }));
...
export function GlowLine() {
  ...
  useEffect(() => {
    ...
    function frame(ts: number) {
      ...
      if (blurRef.current)     blurRef.current.setAttribute("stdDeviation", String(blurAmb));
    ...
    <div className="absolute inset-0 pointer-events-none select-none overflow-hidden">
      ...
      >
        <defs>
          ...
          <linearGradient id="lineColorGrad" x1="0" y1="0" x2="1" y2="0" gradientUnits="objectBoundingBox">
            <stop offset="0%"   stopColor="hsl(221 83% 82%)" />
            <stop offset="30%"  stopColor="hsl(348 83% 80%)" />
            <stop offset="60%"  stopColor="hsl(262 60% 85%)" />
          ...
          {/* Ambient glow filter */}
          <filter id="glowAmbient" x="-20%" y="-200%" width="140%" height="500%">
