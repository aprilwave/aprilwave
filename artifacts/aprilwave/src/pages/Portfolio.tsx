...
import { PresetList } from "@/components/sound-design";
  ...
  return (
    <>
      ...
      <section className={`pt-32 pb-20 px-6 max-w-7xl mx-auto w-full relative z-10 min-h-screen transition-all duration-300 ${showTCG ? "blur-[4px] pointer-events-none select-none" : ""}`}>
        ...
        {!hasTracks && activeCategory !== "Synth Sound Design" ? (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
        ...
        ) : activeCategory === "Synth Sound Design" ? (
          <PresetList />
        ) : (
          /* 2-column layout: playlist on left, player on right */
          <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-6 items-start">
            {/* Playlist */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="max-h-64 lg:max-h-[400px] overflow-y-auto">
                {shuffledTracks.map((track, index) => {
                  const isActive = ctxTrackName === track.title && isCategoryPlaying;
                  return (
                    <motion.button
                      key={track.id}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-display text-sm font-semibold truncate flex-1">{track.title}</span>
                        {track.isPersonalWork ? (
                          <span className="text-[10px] font-medium uppercase tracking-wider px-2 py-0.5 rounded-full bg-primary/15 text-primary flex-shrink-0">
                            personal work
                          </span>
                        ) : null}
                      </div>
